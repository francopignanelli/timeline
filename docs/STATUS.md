...
**App version**: 3.1 (DECISIONS #44 — bump the second number for a small change, the first for a bigger one; shown next to the logo everywhere it renders). History: 1.0 → 2.0 on 2026-08-21 (a large batch: upload cleanup, dark mode, Timeline→Timelines rename, Terms page, delete confirmation, canvas rework). 2.0 → 2.1: canvas wheel handler no longer swallows scroll events inside the Add Milestone/Stage dialogs on short viewports. 2.1 → 2.2: `Dialog`'s backdrop-click-to-close no longer fires for a click or drag on the dialog's own scrollbar (DECISIONS #56). 2.2 → 2.3: Dashboard splits Mine from Shared with me; dark mode is now the default for a first-time visitor (DECISIONS #57). 2.3 → 3.0: Edit Timeline (name/start/end), automatic timeline boundary expansion on linking a Milestone/Stage, and the Welcome Tutorial (DECISIONS #59–61) — a big enough batch of new capability to warrant the first-number bump, not the second. 3.0 → 3.1: a modal no longer leaks pointer events to the canvas behind it, and "Ongoing" now actually clears a stored end date (DECISIONS #62–63) — two user-reported bugs that shared one root cause plus one server-side serialization defect.
