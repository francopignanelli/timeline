...
## 62. A modal stops pointer events at its own boundary (2026-08-21)
**Choice**: `Dialog` cuts propagation of `pointerdown`/`pointermove`/`pointerup` at the `<dialog>` element, and also sets `select-text` on itself. `TimelineCanvas.onPointerDown` additionally skips any event originating inside a `dialog`, matching the guards its wheel and keydown handlers already carried.
**Why**: Two separate user-reported bugs turned out to be one root cause. A native `<dialog>` renders in the top layer but stays where it is in the DOM, and the Add Milestone/Add Stage dialogs are rendered *inside* the canvas — so a pointerdown in one of them reached the canvas's own handler, which called `setPointerCapture()` on the canvas container. That (a) panned the timeline behind the open modal while the user dragged to select text in it, and (b) silently broke every checkbox in those dialogs: with the pointer captured by an ancestor, `pointerup` retargets away from the input, so the browser never synthesises the `click` that toggles it. The dead Stage "Ongoing" control was this, not a form-state bug. Verified by A/B in a real browser with trusted clicks: without the fix `panned=true, checked=false`; with it `panned=false, checked=true`.
**Important alternative**: portal the dialogs out of the canvas subtree. **Why not chosen**: it fixes only the dialogs that happen to live inside the canvas today and leaves the same trap set for the next component that renders a modal inside an interactive container. Cutting propagation at the modal boundary states the actual rule — a modal is modal — once.

## 63. "Ongoing" removes a stored end date server-side, derived not trusted (2026-08-21)
**Choice**: `updateOwnStage`/`updateOwnTimeline` derive `ongoing` from the merged entity and, when it is true, drop `end` from the write patch and pass `['end']` to the repository, which now supports a `remove` list and emits `REMOVE` alongside `SET`.
**Why**: A PATCH body cannot express "clear this field" — the client sends `end: undefined` and `JSON.stringify` strips the key, so "clear it" and "leave it alone" arrive identical. The pre-existing `'end' in patch` check could therefore never be true for a clear. Result: switching a Stage or Timeline that had an end date to ongoing merged `ongoing: true` with the *old* `end`, which `checkTemporalRange` rejects — the save threw before reaching the repository and the UI could only show a generic error. Deriving the removal from `ongoing` (rather than trusting the body to state it) keeps the invariant "an ongoing range has no end date" enforced in one place per module.
**Important alternative**: accept an explicit `end: null` sentinel over the wire. **Why not chosen**: it widens the shared schema's contract for every consumer to encode something the server can already infer, and a client that forgot to send the sentinel would silently reintroduce the same inconsistency.
